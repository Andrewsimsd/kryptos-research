...
def verify(run):
 ...
 manifest=load(run/'manifest.json');spec_path=ROOT/'experiments'/f"{manifest['experiment_id']}.json";spec=load(spec_path);public=load(run/'public-cases.json');private=load(run/'controller-private.json');attack=load(run/'attacker-output.json');reveal=load(run/'seed-reveal.json')
 ...
 expected_manifest={'schema_version':1,'experiment_id':spec['experiment_id'],'run_id':str(run.relative_to(ROOT)),'spec_sha256':hashlib.sha256(spec_path.read_bytes()).hexdigest(),'seed_commitment_sha256':spec['seed_commitment_sha256'],'evidence_file_hashes':spec['evidence_file_hashes'],'implementation_paths':spec['implementation_paths'],'public_case_count':160,'operation_unit':spec['operation_unit'],'operation_cap':spec['calibration_template_check_cap'],'wall_time_cap_seconds':spec['wall_time_cap_seconds'],'binary_sha256':spec['expected_binary_sha256']}
 if manifest!=expected_manifest:raise ValueError('complete manifest mismatch')
 ...
 expected_attack_metadata={'schema_version':1,'raw_parameter_tuples':65856,'canonical_coordinate_maps':200,'canonical_templates':2192,'template_counts':{'autokey':192,'progressive':1600,'repeating_interrupted':400},'operations':spec['calibration_template_check_cap']}
 for key,value in expected_attack_metadata.items():
  if attack.get(key)!=value:raise ValueError('attacker metadata mismatch: '+key)
 if set(attack)!=(set(expected_attack_metadata)|{'cases'}):raise ValueError('attacker output fields mismatch')
 if not isinstance(attack['cases'],list) or [x.get('id') for x in attack['cases']]!=[x['id'] for x in public['cases']]:raise ValueError('attacker case identity/order mismatch')
 if any(set(x)!={'id','templates_checked','survivors'} or x['templates_checked']!=2192 for x in attack['cases']):raise ValueError('attacker per-case metadata mismatch')
 ...
 phased=[x for x in aliases if x['family'] in ('repeating_interrupted','progressive')]
 autokey=[x for x in aliases if x['family']=='autokey']
 if any(x['phase'] not in (0,x['period']-1) for x in phased):raise ValueError('phase is not a registered endpoint')
 if any('phase' in x for x in autokey):raise ValueError('autokey raw model has an inapplicable phase')
 phase_roles={
  family:sorted({'zero' if x['phase']==0 else 'period_minus_one' for x in phased if x['family']==family})
  for family in ('progressive','repeating_interrupted')
 }
 alias_coverage={
  'phase_endpoint_roles_by_family':phase_roles,
 ...
 expected_alias={'phase_endpoint_roles_by_family':{'progressive':['period_minus_one','zero'],'repeating_interrupted':['period_minus_one','zero']},'origins':[0,25],'equations':['beaufort','variant_beaufort','vigenere'],'increments_including_alias':[0,1,25],'reset_subsets':[(),(4,),(4,35),(4,66),(35,),(35,66),(66,)]}
