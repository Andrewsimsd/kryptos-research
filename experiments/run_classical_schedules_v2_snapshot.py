...
def generate(seed):
 ...
 for i in range(40):
  source=public[i*3]; case=json.loads(json.dumps(source)); case['id']=f'tamper-{i:03}'
  j=rng.pick(len(CRIB_POS)); old=ord(case['cribs'][j]['plaintext'])-65; case['cribs'][j]['plaintext']=chr(65+(old+1+rng.pick(25))%26)
  public.append(case); private.append({'id':case['id'],'cohort':'tamper','planted_template_id':private[i*3]['planted_template_id'],'source_id':source['id'],'tamper_position':case['cribs'][j]['position']})
