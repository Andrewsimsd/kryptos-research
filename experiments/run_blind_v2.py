...
def apply_final_wall_cap(completion, start, limit, clock=time.monotonic):
    """Classify the entire measured run after artifact hashing, before writing."""
    elapsed = clock() - start
    completion["elapsed_seconds"] = elapsed
    if elapsed > limit:
        completion.update(status="timeout", exit_status=1,
                          error=f"final wall-time cap exceeded: {elapsed:.6f} > {limit} seconds")
        completion.pop("targets_pass", None)
    return completion
