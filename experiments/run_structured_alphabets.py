#!/usr/bin/env python3
"""Preserve the registered exhaustive structured-alphabet experiment."""
...
def execute(output, specification, versions):
    """Run all stages within the registered wall-time and preserve their logs."""
    inputs = [
        ROOT / "Cargo.toml",
        ROOT / "Cargo.lock",
        ROOT / "experiments/STRUCTURED-ALPHABETS-0001.json",
    ]
    ...
    commands = [
        ...
        ("evaluate", [str(binary), "structured-alphabets", "fixtures/structured-alphabets-request.json"], "structured-alphabets.json"),
        ("verify", [sys.executable, "verification/verify_structured_alphabets.py", "--report", str(output / "structured-alphabets.json"), "--request", "fixtures/structured-alphabets-request.json"], "verification.json"),
    ...
    try:
        for name, command, stdout_name in commands:
            ...
            if process.returncode:
                raise RuntimeError(
                    f"stage {name} failed with exit {process.returncode}; inspect its stderr artifact"
                )
        ...
        if (
            verification["models_checked"] != specification["candidate_model_count"]
            or verification["equations_checked"] != specification["evaluation_cap"]
        ):
            raise RuntimeError("verification did not cover the complete registered domain")
        completion.update(
            status="completed",
            exit_status=0,
            ...
            counters={
                ...
                for name in (
                    "models_checked", "equations_checked", "rejections", "survivors",
                    "best_match_count", "best_model_count",
                )
            },
            conclusion=(
                "Exact compatibility decision for the registered four-order family only; "
                "no plaintext or historical mechanism recovered."
            ),
        )
...
def main(argv=None):
    ...
    try:
        specification = json.loads(
            (ROOT / "experiments/STRUCTURED-ALPHABETS-0001.json").read_text(encoding="utf-8")
        )
        verify_inputs(specification, ROOT)
        versions = {
        ...
        print(f"structured-alphabet experiment failed: {error}", file=sys.stderr)
        return 1
