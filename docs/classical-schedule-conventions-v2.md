...
schedule. Alias cases are scored against their canonical template. The attacker
sees ciphertext and 24 aligned letters only. Its seed commitment is frozen before generation; the
...
The controller encrypts these raw parameters directly. It independently derives
the canonical seed and requires the raw and canonical models to produce the
same 97 ciphertext letters. For variant Beaufort, key negation maps progressive
increment $d$ to $-d\pmod{26}$ and negates the phase-shifted, origin-adjusted
seed. Increment zero maps to the matching coordinate-map template.

The only registered scientific operation is one complete symbolic check of one
canonical template against one case. The exact cap is therefore 2,192 per case,
350,720 for 160 calibration cases, and 2,192 for K4. The runner refuses partial
output or any count other than those totals. Witness construction and verifier
reencryption counts are disclosed separately and are not added to the search
operation count. The wall-time cap is 3,600 seconds and external spend is zero.
Earlier registrations that declared unimplemented elementary-operation caps
are preserved as superseded and do not support the accepted result.
