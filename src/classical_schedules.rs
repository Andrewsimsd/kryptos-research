...
impl Request {
    fn validate(&self) -> Result<(), CipherError> {
        ...
        for case in &self.cases {
            ...
            if case.ciphertext.len() != 97 {
                return Err(parameter("ciphertext", "must contain exactly 97 letters"));
            }
...
fn solve(case: &Case, template: &Template) -> Option<Survivor> {
    match template {
        Template::Coordinate {
            ...
            finish_free(case, template, *period, required, *beaufort)
        }
...
fn finish_free(
    ...
    witness(case, template, fixed, &seed, plaintext, &keys)
}
...
fn witness(
    ...
    let (family, equation, period, increment, resets, feedback) = match template {
        Template::Coordinate {
            period,
            resets,
            beaufort,
...
fn key_stream(
    ...
    match template {
        ...
        Template::Progressive {
            ...
            .map(|i| {
                let step = u8::try_from(i / period % 26).expect("reduced modulo 26");
                add(seed[i % period], mul(step, *increment))
            })
            .collect(),
...
mod tests {
    ...
    fn sign_phase_origin_and_zero_increment_equivalences_are_explicit() {
        ...
        let repeat: Vec<_> = (0_u8..97).map(|i| i % 7).collect();
        let progressive: Vec<_> = (0_u8..97).map(|i| add(i % 7, mul(i / 7, 0))).collect();
        assert_eq!(repeat, progressive);
    }
    ...
    fn distinct_nonzero_progressions_and_autokey_equations_are_not_collapsed() {
        assert_ne!(
            (0_u8..97)
                .map(|i| add(i % 3, i / 3 % 26))
                .collect::<Vec<_>>(),
            (0_u8..97)
                .map(|i| add(i % 3, mul(i / 3 % 26, 2)))
                .collect::<Vec<_>>()
        );
    ...
    fn underconstrained_survivors_have_zero_filled_exact_witnesses() {
        let request = Request {
            schema_version: 1,
            cases: vec![case(&"B".repeat(97), &[(0, 'A')])],
            operation_cap: 2_192,
        };
    ...
    fn invalid_requests_and_cap_edges_are_rejected() {
        let valid = case(&"A".repeat(97), &[(0, 'A')]);
        for request in [
            Request {
                schema_version: 2,
            ...
            Request {
                schema_version: 1,
                cases: vec![case(&"A".repeat(96), &[(3, 'A')])],
                operation_cap: 2192,
            },
            Request {
                schema_version: 1,
                cases: vec![case(&"A".repeat(97), &[(0, 'A'), (0, 'B')])],
                operation_cap: 2192,
            },
        ] {
        ...
        for length in [96, 98] {
            assert!(
                evaluate(&Request {
                    schema_version: 1,
                    cases: vec![case(&"A".repeat(length), &[])],
                    operation_cap: 2_192,
                })
                .is_err()
            );
